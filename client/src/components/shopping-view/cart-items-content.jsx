...
function UserCartItemsContent({ cartItem }) {
    ...
    function handleUpdateQuantity(getCartItem, typeOfAction) {
        ...
        dispatch(updateCartQuantity({
            ...
            if (data?.payload?.success) {
                toast({
                    title: typeOfAction === 'plus' ? 'Increased item quantity' : 'Decreased item quantity'
                })
            }
        })
    ...
    return (
        <div className="flex items-center space-x-4">
            <img
                src={cartItem?.image} alt={cartItem?.title} className="w-20 h-20 rounded object-cover "
            />
            <div className="flex-1">
                <h3 className="font-extrabold">{cartItem?.title}
